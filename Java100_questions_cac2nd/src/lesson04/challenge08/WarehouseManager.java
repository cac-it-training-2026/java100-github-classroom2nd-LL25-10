...
public class WarehouseManager {
	...
	public static void main(String[] args) {
		...
		int[] newABK = new int[10];
		int index1 = 0;
		int index2 = 0;
		for (int i = 0; i < 5; i++) {
			newABK[i] = ABKosanArray1[i];
			newABK[i + 5] = ABKosanArray2[i];
		}
		for (int i = 0; i < newABK.length; i++) {
			if (newABK[i] % 2 != 0) {
				ABKosanArray1[index1++] = newABK[i];
			} else {
				ABKosanArray2[index2++] = newABK[i];
			}
